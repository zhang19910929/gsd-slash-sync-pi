...
// Subagents cannot be enumerated over RPC (get_commands covers extension
// commands, prompt templates and skills only), so the installed side is checked
// from disk: pi-subagents scans <agentDir>/agents/**/*.md, and the generated
// files must be there, marked, and named as the roles GSD workflows spawn.
// Discovery itself is covered hermetically in verify.mjs.
const agentsDir = path.join(agentDir, 'agents');
const installedAgents = fs.existsSync(agentsDir)
  ? fs.readdirSync(agentsDir).filter((f) => f.startsWith('gsd-') && f.endsWith('.md') && !f.includes('.compact'))
  : [];
if (installedAgents.length !== 35) {
  failures.push(`${agentsDir} holds ${installedAgents.length} generated agents (expected 35) — run /gsd-sync`);
}
for (const role of ['gsd-planner', 'gsd-executor', 'gsd-verifier']) {
  const file = path.join(agentsDir, `${role}.md`);
  if (!fs.existsSync(file)) {
    failures.push(`${role}.md is not installed in ${agentsDir}`);
    continue;
  }
  const head = fs.readFileSync(file, 'utf8').slice(0, 4096);
  if (!head.includes(`<!-- generated by ${_internals.GENERATOR}`)) failures.push(`${role}.md is not a generated file`);
  if (!head.includes(`name: "${role}"`)) failures.push(`${role}.md does not declare its pi-subagents name`);
}

console.log('pi end-to-end check');
console.log(`  extension   ${extensionPath} (exists: ${fs.existsSync(extensionPath)})`);
...
console.log(`  agents      ${installedAgents.length} generated in ${agentsDir}`);
